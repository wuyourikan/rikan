...
public class UserCenter extends Activity{
	...
	private ListViewZatanAdapter lvBlogAdapter;
	private List<Zatan> lvBlogData = new ArrayList<Zatan>();
    ...
    private void initLvBlog() {
    	...
    	lvBlogAdapter = new ListViewZatanAdapter(this, ZatanList.CATALOG_USER, lvBlogData, R.layout.zatan_listitem); 
        ...
        mLvBlog.setOnItemClickListener(new AdapterView.OnItemClickListener() {
        	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        		...
        		TextView txt = (TextView)view.findViewById(R.id.zatan_listitem_title);
        		Zatan blog = (Zatan)txt.getTag();
        		//跳转
        ...
        mLvBlog.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
			public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
        		...
        		if(view instanceof TextView){
        			_blog = (Zatan)view.getTag();
        		}else{
        			TextView txt = (TextView)view.findViewById(R.id.zatan_listitem_title);
            		_blog = (Zatan)txt.getTag();
        		} 
        		if(_blog == null) return false;
        		
        		final Zatan blog = _blog;
		...
		new Thread(){
				public void run() {
					...
					if(action == UIHelper.LISTVIEW_ACTION_REFRESH || action == UIHelper.LISTVIEW_ACTION_SCROLL)
						isRefresh = true;
					try {
						ZatanList bloglist= ((AppContext)getApplication()).getUserBlogList(_hisuid, _hisname, pageIndex, isRefresh);
	...
	/*private void lvBlogHandleMessage(Message msg){
		if(msg.what >= 0){
			ZatanList bloglist = (ZatanList)msg.obj;
			...
			case UIHelper.LISTVIEW_ACTION_SCROLL:
				...
				if(lvBlogData.size() > 0){
					for(Zatan blog1 : bloglist.getBloglist()){
						boolean b = false;
						for(Zatan blog2 : lvBlogData){
