...
public class Zhuanti extends Entity{
	...
	public static Zhuanti parse(InputStream inputStream) throws IOException, AppException {
			    ...
			    switch(evtType){ 
			    	case XmlPullParser.START_TAG:
			    		...
			    		{	
				            ...
				            {			            	
				            	zhuanti.setUrl(xmlParser.nextText());
				            }
				            ...
				            {			            	
				            	zhuanti.setBody(xmlParser.nextText());
				            }
				            ...
				            {			            	
				            	zhuanti.setFavorite(StringUtils.toInt(xmlParser.nextText(),0));		            	
				            }
				            //标签
				            else if(tag.equalsIgnoreCase("tags"))
				            {
				            	zhuanti.tags = new ArrayList<String>();
				            }
				            else if(zhuanti.getTags() != null && tag.equalsIgnoreCase("tag"))
				    		{
				            	zhuanti.getTags().add(xmlParser.nextText());
				    		}
				            //通知信息
				            else if(tag.equalsIgnoreCase("notice"))
				    		{
				            	zhuanti.setNotice(new Notice());
				    		}
				            else if(zhuanti.getNotice() != null)
				    		{
				    			if(tag.equalsIgnoreCase("atmeCount"))
					            {			      
				    				zhuanti.getNotice().setAtmeCount(StringUtils.toInt(xmlParser.nextText(),0));
					            }
					            else if(tag.equalsIgnoreCase("msgCount"))
					            {			            	
					            	zhuanti.getNotice().setMsgCount(StringUtils.toInt(xmlParser.nextText(),0));
					            }
					            else if(tag.equalsIgnoreCase("reviewCount"))
					            {			            	
					            	zhuanti.getNotice().setReviewCount(StringUtils.toInt(xmlParser.nextText(),0));
					            }
					            else if(tag.equalsIgnoreCase("newFansCount"))
					            {			            	
					            	zhuanti.getNotice().setNewFansCount(StringUtils.toInt(xmlParser.nextText(),0));
					            }
				    		}
			    		}
			    		break;
