...
public class ApiClient {
	...
	public static String makeStaticURL(String p_url, Map<String, Object> params) {
		StringBuilder url = new StringBuilder(p_url);
		
		Object typeObj = params.get("type");
		Object catalogObj = params.get("catalog");
		Object idObj = params.get("id");
		Object pageIndexObj = params.get("pageIndex");
		Object headImg = params.get("headImg");
	
		int type = StringUtils.toInt(typeObj);
		int catalog = StringUtils.toInt(catalogObj);
		int id = StringUtils.toInt(idObj);
		int pageIndex = StringUtils.toInt(pageIndexObj);
		
		String typeString = TypeHelper.getTypeName(type);
		String catalogString = TypeHelper.getCatalogName(type,catalog);
		...
		if(catalogObj != null)
			url.append("/" + catalogString);		
		if(idObj != null)
			url.append("/" + id);
		if(pageIndexObj != null && headImg == null)
	...
	{	
		String res;
		try {
			if(url.contains("list/special")){
				InputStream in = appContext.getResources().openRawResource(R.raw.special_list); 
			...
			else if(url.contains("list/article")) {
				InputStream in = appContext.getResources().openRawResource(R.raw.article_list); 
			...
			else if(url.contains("img.xml")) {
				InputStream in = appContext.getResources().openRawResource(R.raw.img); 
			...
			else if(url.contains("list/topic")) {
				InputStream in = appContext.getResources().openRawResource(R.raw.topic_list); 
			...
			else if(url.contains("item/article")) {
				InputStream in = appContext.getResources().openRawResource(R.raw.article_detail); 
			...
			else if(url.contains("item/topic")) {
				InputStream in = appContext.getResources().openRawResource(R.raw.topic_detail); 
			...
			else if(url.contains("item/special")) {
				InputStream in = appContext.getResources().openRawResource(R.raw.special_detail); 
	...
	private static InputStream http_gets(AppContext appContext, String url) throws AppException {	
		...
		if(responseBody.contains("result") && responseBody.contains("errorCode") && appContext.containsProperty("user.uid")){
			try {
				...
				res.parse(new ByteArrayInputStream(responseBody.getBytes()));	
	 ...
	 * @throws AppException
	 */
	private static InputStream http_post(AppContext appContext, String url, Map<String, Object> params, Map<String,File> files) throws AppException {
		//System.out.println("topic_url==> "+url);
		String cookie = getCookie(appContext);
		String userAgent = getUserAgent(appContext);
        ...
        for(String name : params.keySet()){
        	...
        	//System.out.println("topic_key==> "+name+"    value==>"+String.valueOf(params.get(name)));
        }
        if(files != null)
        for(String file : files.keySet()){
        	...
        	//System.out.println("topic_key_file==> "+file);
        }
...
/*	public static Result updatePortrait(AppContext appContext, int uid, File portrait) throws AppException {
	 ...
	 * @return
	 */
	public static void httpGet(Base base, AppContext appContext, Map<String, Object> map) throws AppException {
		String getUrl = base.getHttpGetUrl(map);
		try{
			base.parse(http_get(appContext, getUrl));	
		}catch(Exception e){
			if(e instanceof AppException)
				throw (AppException)e;
			throw AppException.network(e);
	...
	/**
	 * POST方法请求数据（可传文件，可自定义URL）
	 * @param Base
	 * @param appContext
	 * @param map
	 * @return
	 */
	public static void httpPost(Entity entity, AppContext appContext, Map<String, Object> map,
								Map<String, File> file, String postUrl) throws AppException {
		if(entity instanceof Base && postUrl == null)
			postUrl = ((Base)entity).getHttpPostUrl();
		try{
			entity.parse(http_post(appContext, postUrl, map, file));	
		}catch(Exception e){
			if(e instanceof AppException)
				throw (AppException)e;
			throw AppException.network(e);
	 ...
	 * @return TopicList
	 */
	/*public static TopicList getTopicListByTag(AppContext appContext, final String tag, final int pageIndex, final int pageSize) throws AppException {
		Map<String,Object> params = new HashMap<String,Object>();
		params.put("tag", tag);
		params.put("pageIndex", pageIndex);
		params.put("pageSize", pageSize);		
		...
		try{
			return TopicList.parse(_post(appContext, URLs.HUATI_LIST, params, null));		
		}catch(Exception e){
			if(e instanceof AppException)
				throw (AppException)e;
			throw AppException.network(e);
	 ...
	 * @return SpecialList
	 */
	/*public static SpecialList getSpecialListByTag(AppContext appContext, final String tag, final int pageIndex, final int pageSize) throws AppException {
		Map<String,Object> params = new HashMap<String,Object>();
		params.put("tag", tag);
		params.put("pageIndex", pageIndex);
		params.put("pageSize", pageSize);		
		...
		try{
			return SpecialList.parse(_post(appContext, URLs.SPECIAL_LIST, params, null));		
		}catch(Exception e){
			if(e instanceof AppException)
				throw (AppException)e;
			throw AppException.network(e);
	 ...
	 * @return Result
	 */ 
/*	public static Result pubTopic(AppContext appContext, Topic topic) throws AppException {
		Map<String,Object> params = new HashMap<String,Object>();
		params.put("uid", topic.getAuthorId());
		params.put("title", topic.getTitle());
		params.put("catalog", topic.getCatalog());
		params.put("content", topic.getBody());
		params.put("isNoticeMe", topic.getIsNoticeMe());				
...
/*	public static Result delMessage(AppContext appContext, int uid, int friendid) throws AppException {
	 ...
	 * @return Result
	 */
/*	public static Result pubComment(AppContext appContext, int catalog, int id, int uid, String content, int isTopicToMyZone) throws AppException {
		Map<String,Object> params = new HashMap<String,Object>();
		params.put("catalog", catalog);
		params.put("id", id);
		params.put("uid", uid);
		params.put("content", content);
		params.put("isTopicToMyZone", isTopicToMyZone);
